...
from .models import ScanContext, ScanFinding
...
def _scan_record(
    ...
    clam = clamav.scan_file(path) if config.clamav.enabled else {"status": "disabled", "signature": None}
    finding.clamav = clam
...
def _deep_extract_worker(path_text: str, out_dir_text: str, sha256: str, quarantine_mode: str, clamav_enabled: bool) -> dict[str, Any]:
    path = Path(path_text)
...
def _triage_interesting(tag_counts: dict[str, int], lex, data: bytes) -> bool:
    if any(tag_counts.get(name, 0) for name in ("/JS", "/JavaScript", "/OpenAction", "/AA", "/Launch", "/EmbeddedFile", "/URI")):
        return True
